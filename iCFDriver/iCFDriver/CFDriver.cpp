#include "CFDriver.h"
using namespace std;
void CFDriver::error(const char *msg)
{
    perror(msg);
    exit(1);
}
...
void CFDriver::recv_cmd(){
    
    int ret = pthread_create(&pth, NULL, pth_recv, this);
}

void CFDriver::enable_send()
{
    treatCmd = false;
}
...
int CFDriver::send_file(std::string file){
    int                     read_len;
    int                     send_len;
    FILE                    *fp;
    char                    revbuf[MAXLINE];
...
int CFDriver::send_data(void *buf, int len, std::string name, int type){
    int     read_len;
    int     send_len;
    int     left;
    int     count = 0;
